...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const article = PILLAR_ARTICLES[slug]
  if (!article) {
    const { data } = await supabase.from('articles').select('title, excerpt, seo_keywords').eq('slug', slug).single()
...
function renderMarkdown(content: string): string {
  // First, remove table separator rows entirely
  let result = content.replace(/^\|[\s-:|]+\|$/gm, '')
  
  result = result
    // Headings - process ### before ## to avoid conflict
    .replace(/^### (.*$)/gm, '<h3>$1</h3>')
    .replace(/^## (.*$)/gm, '<h2>$1</h2>')
    ...
    // Wrap consecutive <tr> rows into tables
    .replace(/(<tr>.*<\/tr>\n?)+/g, (block) => {
      const rows = block.trim().split('\n').filter(r => r.trim())
      if (rows.length === 0) return ''
      const header = rows[0].replace(/<td>/g, '<th>').replace(/<\/td>/g, '</th>')
      const body = rows.slice(1)
      return `<table class="spec-table"><thead>${header}</thead><tbody>${body.join('')}</tbody></table>`
    })
    .replace(/^- (.*)/gm, '<li>$1</li>')
    .replace(/(<li>.*<\/li>\n?)+/g, (block) => `<ul>${block}</ul>`)
...
export default async function ArticlePage({ params }: Props) {
  const { slug } = await params
  const supabase = await createSupabaseServer()
  let article = PILLAR_ARTICLES[slug]
  ...
  const schema = {
    ...
    mainEntityOfPage: { '@type': 'WebPage', '@id': `https://hoistmarket.com/knowledge-base/${slug}` },
  }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
              ...
              <div className="space-y-4 lg:sticky lg:top-24">
                ...
                <div className="bg-navy-900 rounded-xl p-6 text-center">
                  ...
                  <Link href="/contact" className="btn-primary btn-sm w-full justify-center">Request a Quote →</Link>
                </div>
