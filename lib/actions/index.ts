...
async function runRFQMatching(rfqId: string, category: string, region: string) {
  ...
  const scored = vendors.map(v => {
    ...
    const vendorCats = v.equipment_categories.map((c: string) => c.toLowerCase())

    // Category match (50 pts)
    if (vendorCats.some((c: string) => c === catLow)) score += 50
    else if (vendorCats.some((c: string) => c.includes(catLow.split(' ')[0]) || catLow.includes(c.split(' ')[0]))) score += 30
