...
const footerLinks = {
  ...
  Company: [
    ...
    { label: 'List Your Company', href: '/vendor/onboard' },
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-2.5 mb-5">
              <div className="w-8 h-8 bg-white rounded flex items-center justify-center flex-shrink-0 overflow-hidden">
                <img src="/logo.svg" alt="HoistMarket" className="w-full h-full object-contain" />
              </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t border-white/5 pt-6">
          ...
          <div className="flex items-center gap-4">
            {[
              { item: 'Privacy Policy', href: '/contact' },
              { item: 'Terms of Use', href: '/contact' },
              { item: 'Editorial Policy', href: '/about' },
            ].map(({ item, href }) => (
              <Link key={item} href={href} className="text-xs text-slate-600 hover:text-slate-400 transition-colors">
                {item}
              </Link>
            ))}
          </div>
